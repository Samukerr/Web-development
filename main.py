...
def restrict_page_register(function):
    @wraps(function)
    def inner_function(*args, **kwargs):
        user_id = current_user.get_id()
        user = Users.query.filter_by(id=user_id).first()
        if user.registeredas != "":
            return abort(403, "Access denied user has already registered!!!")
        else:
            return function(*args, **kwargs)
    return inner_function


def restrict_page_vote(function):
    @wraps(function)
    def inner_function(*args, **kwargs):
        user_id = current_user.get_id()
        user = Users.query.filter_by(id=user_id).first()
        if user.party_vote != "":
            return abort(403, "Access denied user has already voted.")
        if user.registeredas == "":
            return abort(403, "Access denied user has not registered.") 
        else:
            return function(*args, **kwargs)
    return inner_function
...
def register(err):
    ...
    if request.method == "POST":
        ...
        registration_data = [check_password_hash(data.id_number, id_number) for data in db.session.query(Registrations).all()]
        if True in registration_data:
        ...
        # checks if user meets the minimum age requirements - 18 yrs
        ...
        db_add(Registrations(name=generate_password_hash(name),
                             surname=generate_password_hash(surname),
                             id_number=generate_password_hash(id_number),
                             address=generate_password_hash(address),
                             date_of_birth=generate_password_hash(date),
                             contact_numbers=generate_password_hash(contact_numbers),
                             proof_of_address=generate_password_hash(proof_of_address),
                             citizenship=generate_password_hash(citizenship),
                             province=generate_password_hash(province),
                             language=generate_password_hash(language),
                             political_party_affiliation=generate_password_hash(political_party_affiliation),
                             disabled=generate_password_hash(disabled),
                             gender=generate_password_hash(gender),
                             married=generate_password_hash(married)))

        user_id = current_user.get_id()
        user = Users.query.filter_by(id=user_id).first()
        registrationid = db.session.query(Registrations).all()[-1]
        user.registeredas = f"{registrationid.id}"
        db.session.commit()
...
def sign_up(err):
    ...
    if request.method == "POST":
        ...
        # checks email or password already in database
...
def sign_in(err):
    if request.method == "POST":
        ...
        if email not in emails:
            return redirect(url_for("sign_in", err="Email*does*not*match*with*any*other*Email*Address*in*the*database!!!"))
...
def vote():
    ...
    if request.method == "POST":
        ...
        register_db_data = Registrations.query.filter_by(id=int(user_db_data.registeredas)).first()
        user_name = user_db_data.name
        user_surname = user_db_data.surname
        user_id_number = register_db_data.id_number
